...
We know $S \subset \mathbb{R}^d$ is a non empty convex set. Let $x, x' \in S$ and $\lambda \in [0,1]$, so here we cover two cases:

\begin{itemize}
    \item \textit{First case}. If $x \in S$, we have by definition that
    
    \begin{equation*}
        \tilde f (\lambda x + (1- \lambda)x') = f (\lambda x + (1- \lambda)x')  \leq \lambda f(x) + (1-\lambda) = \lambda \tilde f(x) + (1-\lambda)
    \end{equation*}
    ...
    \begin{equation*}
        \lambda \tilde f (x) + (1 - \lambda) \tilde f (x') = + \infty,
    \end{equation*}

    which we can say it is directly convex, $\tilde f (y) \leq + \infty$, $\forall y$. Note that this also applies to $x'$.
\end{itemize}

Since both cases satisfy convexity, we proof that this function is convex.
