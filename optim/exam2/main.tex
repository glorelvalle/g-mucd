...
%    \end{cases}.
%\]
Now we compute the proximal operator of $f$ as $\mbox{prox}_f = (I + \partial f)^{-1}$ . Note that $f$ is differentiable, so $\partial f (x) = \left\{ f'(x) \right\}$, a single value set. Using this, and denoting it as a function:
...
where the values at $(-3, -1)$ and $(1, 3)$ corresponds to the discontinuities of $(I + \partial f) (x)$. Finally, we ilustrate $(\text{I} + \partial f)^{-1}(x)$ for a better comprehension.\\
